...
import { Calendar, ClipboardCheck, BarChart3, Lock } from 'lucide-react'
...
export function Home({ appName, onEnterAdmin }: HomeProps) {
  ...
  const features: { title: string; desc: string; icon: React.ReactNode }[] = [
    ...
    },
    {
      ...
      icon: <BarChart3 className="w-4 h-4" strokeWidth={1.8} />,
    },
  ]
  ...
  return (
    <div className="min-h-screen flex flex-col bg-background">
      ...
      <main className="flex-1 max-w-5xl w-full mx-auto px-4 sm:px-6 py-10">
        {checking ? (
          <div className="flex flex-col items-center justify-center py-20">
            <div className="w-10 h-10 border-3 border-border border-t-brand-500 rounded-full animate-spin mb-3" />
            <p className="text-sm text-muted-foreground/70">正在检查登录状态…</p>
          </div>
        ) : (
        <div className="grid md:grid-cols-2 gap-8 items-center">
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-3">
              {features.map((f) => (
                <div key={f.title} className="bg-background rounded-lg border border-border p-4">
          ...
          <div className="w-full max-w-sm mx-auto">
            <div className="text-center mb-6">
              ...
              <h2 className="text-xl font-semibold text-foreground">{'管理员登录'}</h2>
              <p className="text-sm text-muted-foreground/70 mt-1">管理员 / 教师登录后台</p>
            </div>
      ...
      {/* 页脚 */}
      <footer className="border-t border-border py-4 text-center text-xs text-muted-foreground/70">
        <span>{appName}</span>
        {GITHUB_URL && (
          <>
            ...
            <a
              ...
              className="text-muted-foreground/70 hover:text-primary transition-colors inline-flex items-center gap-1 align-middle"
            >
